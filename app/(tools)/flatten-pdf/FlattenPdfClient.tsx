...
export function FlattenPdfClient() {
  ...
  return (
    <ToolShell icon={<Layers2 className="h-5 w-5" aria-hidden="true" />} title="Flatten PDF" description="Re-save a PDF as a static document.">
      <div className="space-y-5">
        ...
        {tool.result ? <DownloadCard blob={tool.result.blob} filename={tool.result.filename} onReset={tool.reset} /> : (
          <ProcessButton disabled={tool.files.length === 0} loading={loading} progress={tool.progress} onClick={() => tool.process(async ([file], onProgress) => {
            ...
            onProgress(100);
            return { blob: uint8ArrayToBlob(bytes, "application/pdf"), filename: "flattened.pdf" };
          })}>
            Flatten PDF
          </ProcessButton>
        )}
